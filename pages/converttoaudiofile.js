import Mp3Component from '../components/Mp3Component';
...
export default function converttoaudiofile() {
	// body...
    ...
    useEffect(()=>{
        if(status==='authenticated'){
            ...
            router.push('/login?redirect=converttoaudiofile');
        }
    },[])
...
<div className="" >
        	...
        	scrollbar-hidden scroll-smooth">
        		<div className="w-full md:px-2 px-4 mt-[55px] z-10 flex md:flex-row flex-col">
            		...
            		className="md:w-[50%] relative md:mt-10 mt-7 rounded-xl">
            			<img src="https://ik.imagekit.io/d3kzbpbila/thejashari_GYeqkfqiq?ik-sdk-version=javascript-1.4.3&updatedAt=1674906142552"
            			alt=""
            			...
            			className="absolute md:blur-xl blur-lg h-full top-0 w-full bg-[#01ded1]"/>
            		</motion.div>
            		<motion.div 
            		initial={{
            			opacity:0.2,
            		...
            		className="md:w-[50%] items-center p-4 md:p-6 md:ml-2 justify-center flex flex-col">
            			<h1 className="md:text-2xl text-lg md:mt-20 mt-5 font-anurati text-center text-gray-200 uppercase tracking-[5px]">Speech and text to audio file</h1>
            			<p 
            			className="text-center text-md md:text-xl mt-5 text-gray-300 font-mono">{process.env.NEXT_PUBLIC_CODE_TEXT}</p>
            			<button 
		...
		<Mp3Component/>
        <Script src="https://code.responsivevoice.org/responsivevoice.js?key=eRB9OsaK"></Script>
	</div>
