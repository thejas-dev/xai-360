...
export default function safety() {
    ...
    useEffect(()=>{
        if(status==='authenticated'){
            ...
            router.push('/login?redirect=safety');
        }
    },[])
            ...
            scrollbar-hidden scroll-smooth">
                <div className="w-full md:px-2 px-4 mt-[55px] z-10 flex md:flex-row flex-col">
                    ...
                    className="md:w-[50%] relative md:mt-10 mt-7 rounded-xl">
                        ...
                        className="absolute md:blur-xl blur-lg h-full top-0 w-full bg-[#0194cc]/60"/>
                    </motion.div>
                    <motion.div 
                    initial={{
                        opacity:0.2,
                    ...
                    className="md:w-[50%] items-center p-2 md:p-6 md:ml-2 justify-center flex flex-col">
                        <h1 className="md:text-2xl text-lg md:mt-20 mt-5 font-anurati text-gray-200 uppercase tracking-[5px] text-center">CONTENT SAFETY</h1>
                        <p 
                        className="text-center text-md md:text-xl mt-5 text-gray-300 font-mono">{process.env.NEXT_PUBLIC_SAFETY_TEXT}</p>
                        <button 
