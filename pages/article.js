...
export default function article() {
    ...
    useEffect(()=>{
        if(status==='authenticated'){
            ...
            router.push('/login?redirect=article');
        }
    },[])
        	...
        	scrollbar-hidden scroll-smooth">
        		<div className="w-full md:px-2 px-4 mt-[55px] z-10 flex md:flex-row flex-col">
            		...
            		className="md:w-[50%] relative md:mt-10 mt-7 rounded-xl">
            			<img src="https://miro.medium.com/max/1080/1*jaElTuNqyKJq4Mt1nLkM4g.png"
            			alt=""
            			...
            			className="absolute md:blur-xl blur-lg h-full top-0 w-full bg-[#b2daf0]"/>
            		</motion.div>
            		<motion.div 
            		initial={{
            			opacity:0.2,
            		...
            		className="md:w-[50%] items-center p-4 md:p-6 md:ml-2 justify-center flex flex-col">
            			<h1 className="md:text-2xl text-lg md:mt-20 mt-5 font-anurati text-gray-200 uppercase tracking-[5px] text-center">article writing</h1>
            			<p 
            			className="text-center text-md md:text-xl mt-5 text-gray-300 font-mono">{process.env.NEXT_PUBLIC_ARTICLE_TEXT}</p>
            			<button 
            			onClick={changeWindow}
            			className="mt-10 rounded-xl bg-gray-100/90 px-3 py-2 text-black font-mono tracking-[3px]">Write Now</button>
            		</motion.div>
