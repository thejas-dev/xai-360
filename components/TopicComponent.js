  ...
  const uploadAudio = async(uploadFileUrl) => {
    	...
    	const interval = setInterval(()=>{
		    ...
		    .then((res) => {
		    	...
		    	if(res.data.status === 'completed'){
		    		...
		    		setCategoryResult(res.data.iab_categories_result);
   ...
   const typeMessageLoading = (element,text) => {
	...
	className="relative min-h-screen hidden scroll-smooth z-0 w-full bg-[#0C0C0C] overflow-x-hidden overflow-y-scroll">
		...
		<Header2 hide="true" token="orange" redirect="topic" />
		<motion.div 
          initial={{
            opacity:0
          }}
    	...
    	scroll-smooth">	
    		<h1 className="text-3xl md:text-4xl mb-3 text-[#FFF] text-shadow-fire mx-auto">Audio <span className="text-sky-500">Topic Detection(incomplete)</span></h1>	
    		...
    		<input 
	    		...
	    		<center>
				<button className={`mt-7 ${uploadedUrl ? "bg-green-600" : "bg-green-700/30" }
			...
			<div id="resultContainer" className="flex flex-col hidden relative border-[1px] border-gray-400 mt-10 min-h-screen rounded-xl text-[#dcdcdc] max-w-[100%] 
			px-3 font-sourceCode py-3 no-scrollbar">
				<h1 className="text-xl font-medium mx-auto mb-5 text-center text-shadow-fire">Topic Detection <span className="text-sky-500">Result</span></h1>
				<div 
