...
export default function DualComponent() {
	// body...
	...
	const [generatedUtterances,setGeneratedUtterances] = useState([]);
  ...
  const uploadAudio = async(uploadFileUrl) => {
    	...
    	const interval = setInterval(()=>{
		    ...
		    .then((res) => {
		    	...
		    	if(res.data.status === 'completed'){
		    		...
		    		if(res.data.utterances === null){
		    			setGeneratedUtterances(res.data.text);
		    		}else{
		    			setGeneratedUtterances(res.data.utterances);
		    		}
   ...
   const typeMessageLoading = (element,text) => {
	...
	className="relative min-h-screen hidden scroll-smooth z-0 w-full bg-[#0C0C0C] overflow-x-hidden overflow-y-scroll">
		...
		<Header2 hide="true" redirect="phone" token="orange"/>
		<motion.div 
          initial={{
            opacity:0
          }}
    	...
    	scroll-smooth">	
    		<h1 className="text-2xl md:text-4xl mb-3 text-[#FFF] text-shadow-fire mx-auto">Transcribe <span className="text-sky-500">Call Recording</span></h1>	
    		<div className="h-[2px] w-[50%] md:w-[25%] mx-auto bg-orange-500 "/>
    		<input 
    			id="file_input"
			...
			<div id="resultContainer" className="flex hidden flex-col relative border-[1px] border-gray-400 mt-10 min-h-screen rounded-xl text-[#dcdcdc] max-w-[100%] 
			px-3 font-sourceCode py-3 no-scrollbar bg-black/30">
				<h1 className="text-xl font-medium mx-auto mb-5 text-center text-shadow-fire">Dual Channel Transcribe <span className="text-sky-500">Result</span></h1>
				{
					generatedText ? 
					""
					:
				...
				<div id="resultBox" className="flex flex-col font-sourceCode mt-5 whitespace-pre-wrap md:overflow-x-hidden overflow-x-scroll">
					{
						typeof generatedUtterances === 'string' ?
						<div className="px-2 py-3 rounded-xl w-full break-all">
							<h1 className="md:text-lg text-md mt-2 font-medium text-[#FFF]">{generatedUtterances}</h1>
						</div>
						:
						<>
						{
							generatedUtterances?.map((utter)=>(
								<div key={utter.text} className="px-2 py-3 rounded-xl w-full break-all">
									<h1 className="md:text-lg text-md mt-2 font-medium text-[#FFF]"><span className={`${utter.speaker === '1' ? 'text-sky-500': utter.speaker === '2' ? 'text-orange-500' : utter.speaker === '3' ? 'text-purple-500' : utter.speaker === '4'? 'text-green-500': utter.speaker === '5' ? 'text-pink-500' : utter.speaker === '6' ? 'text-yellow-500': 'text-red-500' }`}>Speaker {utter.speaker}</span> <span className="text-gray-500/70">({convertToMinutesAndSeconds(utter.start)} - {convertToMinutesAndSeconds(utter.end)})</span> :- {utter.text}</h1>
								</div>
