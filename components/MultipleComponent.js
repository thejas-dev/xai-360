  ...
  const uploadAudio = async(uploadFileUrl) => {
   ...
   const typeMessageLoading = (element,text) => {
	...
	className="relative min-h-screen hidden scroll-smooth z-0 w-full bg-[#0C0C0C] overflow-x-hidden overflow-y-scroll">
		...
		<Header2 hide="true" redirect="multiplespeakers" token="orange"/>
		<motion.div 
          initial={{
            opacity:0
          }}
    	...
    	scroll-smooth">	
    		<h1 className="text-2xl md:text-4xl mb-3 text-[#FFF] text-shadow-fire mx-auto">Multiple <span className="text-sky-500">Speakers Transcribing</span></h1>	
    		...
    		<div className="mt-7 md:w-[70%] w-[95%] mx-auto rounded-xl border-[1px] border-gray-400 px-3 bg-black/30 py-3">
    			...
    			<p className="text-md mt-3 font-sourceCode text-[#FFF]">
    				Our Speaker Detection service can detect upto 10 unique speakers.In order to be identified as a unique speaker, a speaker need to approximately speak for atleast 30 seconds over the audio file.
    				<br/>
    				If a person doesn't speak more than 30 seconds like just 'hmmm' or 'okay' the model will find difficulty in identifying	them as a unique speaker.
    			</p>
    		</div>
    		<input 
    			id="file_input"
			...
			px-3 font-sourceCode py-3 no-scrollbar">
				<h1 className="text-xl font-medium mx-auto mb-5 text-center text-shadow-fire">Audio Detection <span className="text-sky-500">Result</span></h1>
				{
					generatedText ? 
					""
					:
				...
				<div id="resultBox" className="flex flex-col font-sourceCode mt-5 whitespace-pre-wrap md:overflow-x-hidden overflow-x-scroll w-full">
					{
						generatedUtterances.map((utter)=>(
							...
							className="px-2 py-3 mt-5 border-[1px] border-gray-400 w-full relative mx-auto bg-black/40 rounded-xl">
								...
								<h1 className="text-xl text-center mx-auto font-medium text-sky-500">
