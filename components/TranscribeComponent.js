  ...
  const uploadAudio = async(uploadFileUrl) => {
    ...
    useEffect(()=>{
    	if(generatedResponse){
	    	...
	    	if(generatedResponse.status==='queued'){
	    		...
	    		typeMessageLoading(loading_text,'Transcribing...')
	    		...
	    		typeMessageMain(element,generatedResponse.text);
   ...
   const typeMessageLoading = (element,text) => {
	...
	className="relative min-h-screen hidden scroll-smooth z-0 w-full bg-[#0C0C0C] overflow-x-hidden overflow-y-scroll">
    	...
    	scroll-smooth">	
    		<h1 className="text-4xl md:text-5xl mb-3 text-[#FFF] text-shadow-fire mx-auto">Transcribe <span className="text-sky-500">Audio</span></h1>	
    		...
    		<input 
	    		...
	    		<center>
				<button onClick={()=>{if(!loader && uploadedUrl )transcribeNow();}} className={`mt-7 ${uploadedUrl ? "bg-green-600" : "bg-green-700/30" }
			...
			className="w-full hidden z-10 px-5 py-2 mt-10 rounded-xl py-2 flex md:px-20 px-6 flex-col justify-center items-center">
				<div className="flex mx-auto mt-3">
					...
					<h1 id="loading_text" className="text-xl text-gray-300">Transcribing...</h1> 
				</div>
			...
			px-3 font-sourceCode py-3 no-scrollbar">
				<h1 className="text-xl font-medium mx-auto mb-5 text-center text-shadow-fire">Audio Transcribe <span className="text-sky-500">Result</span></h1>
				<div 
