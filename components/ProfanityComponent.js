...
export default function ProfanityComponent() {
	// body...
	const [infoVisiblity,setInfoVisiblity] = useState(true);
	const [prompt,setPrompt] = useState('');
	const [loader,setLoader] = useState('');
	const [language,setLanguage] = useState('English');
	const [moderateLevel,setModerateLevel] = useState('strict');
	...
	className="relative min-h-screen hidden scroll-smooth z-0 w-full bg-[#0C0C0C] overflow-x-hidden overflow-y-scroll">
		...
		<Header2 hide="true"  redirect="profanity" token="blue" />
		<motion.div 
          initial={{
            opacity:0
          }}
    	...
    	scroll-smooth">	
    		<h1 className="text-4xl md:text-5xl mb-3 text-[#FFF] text-shadow-fire mx-auto">Profanity <span className="text-sky-500">Filter</span></h1>	
    		...
    		<div className="w-full z-10 mt-10 rounded-xl px-2 py-2 flex md:flex-row flex-col">
    			...
    			className="md:w-[50%] px-3 py-4 md:items-center flex md:flex-row flex-col border-l-2 border-gray-400 focus-within:border-sky-400 w-full">
    				<h1 className="text-xl text-[#FFF] font-semibold ml-3">Text language <span className="text-red-500">*</span></h1>
    				<div 
    				id="language_box"
    		...
    		<div className="w-full z-10 rounded-xl px-2 py-2 flex flex-col">
    			...
    			className="px-3 py-4 md:items-center flex md:flex-row flex-col border-l-2 border-gray-400 focus-within:border-orange-400 w-full">
    				<h1 className="text-xl text-[#FFF] whitespace-nowrap font-semibold ml-3">Text <span className="text-orange-500">Content</span> <span className="text-red-500">*</span> </h1>
    				<div 
    				id="prompt_box"
    				className="w-full h-[170px] md:h-[120px] md:ml-[100px] md:mt-0 mt-3 items-center flex px-3 py-2 rounded-xl border-[1px] border-gray-400 focus-within:border-orange-400 flex-col">
						<textarea 
						value={prompt}
						placeholder="Text content to filter"
    			...
    			id="filter_container"
    			className="px-3 py-4 md:items-center flex md:flex-row flex-col border-l-2 border-gray-400 focus-within:border-orange-400 w-full">
    				<h1 className="text-xl text-[#FFF] whitespace-nowrap font-semibold ml-3"><span className="text-purple-500">Specific</span> words to flag</h1>
    				<div 
    				id="filter_box"
    				className="w-full md:ml-9 md:mt-0 mt-3 flex px-3 py-2 rounded-xl border-[1px] 
    				border-gray-400 focus-within:border-orange-400 flex-col">
						<input 
						value={filterName}
						placeholder="Words to flag as important"
						onChange={(e)=>setFilterName(e.target.value)}
						className="border resize-none h-full text-[#FFF] text-md rounded-lg block w-full p-2.5 bg-gray-700/60 
						border-sky-600 placeholder-gray-400 outline-none"/>
    				</div>
    			</div>
    			<center>
					<button onClick={()=>{if(!loader)filterNow();}} className="mt-7 bg-green-700 
