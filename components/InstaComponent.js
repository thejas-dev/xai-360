...
export default function InstaComponent() {
	...
	const generateCaptions = async() => {
		const response = await openai.createCompletion({
			model: "text-davinci-003",
			prompt:`Can you suggest some captions for a ${category} video showcasing ${prompt} with ${keywords}`,
			temperature: 0.3,
	  		max_tokens: 500,
	  		top_p: 1,
	  		frequency_penalty: 0.5,
	...
	const generateTags = async() => {
		const response = await openai.createCompletion({
			model: "text-davinci-003",
			prompt:`Can you suggest some tags for a ${category} video showcasing ${prompt} with ${keywords}`,
			temperature: 0.3,
	  		max_tokens: 500,
	  		top_p: 1,
	  		frequency_penalty: 0.5,
