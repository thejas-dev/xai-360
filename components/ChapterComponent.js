  ...
  const uploadAudio = async(uploadFileUrl) => {
    ...
    useEffect(()=>{
    	if(chaptersArray.length>0){
    		setGeneratedText(generatedResponse.chapters[0].gist);
    		for(var i = 0;i<generatedResponse.chapters.length;i++){
    			const gistElement = document.getElementById(`gist-${i}`)
    			typeMessageMain(gistElement,generatedResponse.chapters[i].gist)
    			const headlineElement = document.getElementById(`headline-${i}`)
    			typeMessageMain(headlineElement,generatedResponse.chapters[i].headline)
    			const summaryElement = document.getElementById(`summary-${i}`)
    			typeMessageSummary(summaryElement,generatedResponse.chapters[i].summary)
    		}
    	}
    },[chaptersArray])
   ...
   const typeMessageLoading = (element,text) => {
	...
	const typeMessageSummary = (element,text) =>{
	...
	className="relative min-h-screen hidden scroll-smooth z-0 w-full bg-[#0C0C0C] overflow-x-hidden overflow-y-scroll">
		<div id="upperDesign" className="fixed w-[150px] animate-pulse md:w-[200px] h-[150px] md:h-[200px] rounded-full bg-[#d040f7]/40 -top-[5%] -right-14 blur-2xl md:-right-20"/>
		<div id="lowerDesign" className="fixed w-[150px] md:w-[200px] h-[150px] md:h-[200px] rounded-full bg-[#d040f7]/40 -bottom-[5%] blur-2xl -left-14 md:-left-20"/>
		...
		<Header2 hide="true" redirect="chapter" token="orange"/>
		<motion.div 
          initial={{
            opacity:0
          }}
    	...
    	scroll-smooth">	
    		...
    		<input 
				...
				w-[150px] mx-auto md:text-xl text-lg text-gray-300 tracking-[3px] px-3 py-2 rounded-full hover:scale-110 active:scale-90 
			...
			px-3 font-sourceCode py-3 no-scrollbar">
				<h1 className="text-xl font-medium mx-auto mb-5 text-center text-shadow-fire">Chapter Detection <span className="text-sky-500">Result</span></h1>
				{
					generatedText ? 
					""
					:
				...
				<div id="resultBox" className="flex font-sourceCode mt-5 whitespace-pre-wrap md:overflow-x-hidden overflow-x-scroll flex-col">
					{
						chaptersArray.map((chap,i)=>(
						<div className="relative mt-7 bg-black/20 border-[1px] border-gray-400 px-3 py-2 rounded-xl">
							<div 
							...
							<h1 className="text-2xl font-bold text-center mx-auto">Chapter - {i+1}</h1>
							<div className="h-[2px] w-[100px] bg-[#d040f7] mt-2 mx-auto"/>
							<h1 className="text-md mt-3 text-gray-500 mx-auto text-center">Starts from {convertToMinutesAndSeconds(chap.start)} to {convertToMinutesAndSeconds(chap.end)}</h1>
							<h1 className="mt-5 text-xl font-medium text-orange-500">Gist</h1>
							<h1 
							id={`gist-${i}`}
							className="text-xl font-sourceCode mt-1 text-[#FFF]"></h1>
							<h1 className="mt-5 text-xl font- text-sky-500">Headline</h1>
							<h1 
							id={`headline-${i}`}
							className="text-lg mt-1 font-sourceCode text-[#FFF]"></h1>
							<h1 className="mt-5 text-xl font- text-yellow-500">Summary</h1>
							<h1 
							id={`summary-${i}`}
							className="text-md font-sourceCode mt-1 text-[#FFF]"></h1>
						</div>
						))
					}
